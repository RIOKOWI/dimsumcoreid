<x-app-layout>
    <div class="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow-md mt-8">
        <h2 class="text-2xl font-semibold text-red-600 mb-6">✏️ Edit Pelanggan</h2>

        <form action="{{ route('pelanggan.update', $pelanggan) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="nama" class="block mb-1 font-medium text-gray-700">Nama <span class="text-red-500">*</span></label>
                <input id="nama" type="text" name="nama" value="{{ old('nama', $pelanggan->nama) }}" required
            ...
            <div>
                ...
                <input id="no_telepon" type="text" name="no_telepon" value="{{ old('no_telepon', $pelanggan->no_telepon) }}" required
            ...
            <div>
                ...
                <input id="instagram" type="text" name="instagram" value="{{ old('instagram', $pelanggan->instagram) }}"
            ...
            <div>
                ...
                <textarea id="alamat" name="alamat" rows="3" required
                    class="w-full px-4 py-2 border rounded-md resize-y focus:outline-none focus:ring-2 focus:ring-red-500 @error('alamat') border-red-600 @enderror">{{ old('alamat', $pelanggan->alamat) }}</textarea>
                @error('alamat')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea id="catatan" name="catatan" rows="3"
                    class="w-full px-4 py-2 border rounded-md resize-y focus:outline-none focus:ring-2 focus:ring-red-500 @error('catatan') border-red-600 @enderror">{{ old('catatan', $pelanggan->catatan) }}</textarea>
                @error('catatan')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
